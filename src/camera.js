...
// These anchor points allow the pose pointcloud to resize according to its position in the input
...
export class Camera {
    ...
    /**
     * Initiate a Camera instance and wait for the camera steam to be ready.
     * 
    ...
    static async setupCamera(cameraParam) {
        ...
        camera.scatterGLEl.style.display = params.STATE.modelConfig.render3D ? 'inline-block' : 'none';
        return camera;
    }
    ...
    drawKeypoint(keypoint) {
        // If the score is null, just show the keypoint
    ...
    /**
     * Draw the skeleton on a body on the video
     * 
     * @param keypoints A list of keypoints
     */
    drawSkeleton(keypoints, poseId) {
        ...
        posedetection.util.getAdjacentPairs(params.STATE.model).forEach(([i, j]) => {
            ...
            const score2 = kp1.score != null ? kp2.score : 1;
            const scoreThreshold = params.STATE.modelConfig.scoreThreshold || 0;

            if (score >= scoreThreshold && score2 >= scoreThreshold) {
                this.ctx.beginPath();
