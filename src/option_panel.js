...
/**
 * Records each flag's default value under the runtime environment and is a constant in runtime.
 */
...
async function showBackendConfigs(folderController) {
    // Clean up the backend configs for the previous model.
    ...
    // The first element of the array is the default backend of the model.
    ...
    await showFlagSettingsgs(folderController, params.STATE.backend);
}

function showModelConfigs(folderController, type) {
    // Clean up the model configs for the previous model.
...
// The PoseNet model config folder contains options for PoseNet config settings.
...
// The network model comfig folder contains options for MoveNet config settings
function addMoveNetControllers(modelConfigFolder, type) {
    ...
    // Set multipose defaults on initial page laad.
    ...
    const typeController = modelConfigFolder.add(params.STATE.modelConfig, 'type', ['lightning', 'thunder', 'multipose']);
    typeController.onChange(type => {
        // Set isModelChanged to true, so that we don't render any result during changing models.
        params.STATE.isModelChanged = true;
        if (type === 'multipose') {
            // Defaults to enable tracking for multipose.
            if (enableTrackingController) {
                enableTrackingController.setValue(true);
            }

            // Defaults to a lower scoreThreshold for multipose
    ...
    scoreThresholdController = modelConfigFolder.add(params.STATE.modelConfig, 'scoreThreshold', 0, 1);
...
// The BlazePose model config folder contains options for BlazePose config settings
function addBlazePoseControllers(modelConfigFolder, type) {
    ...
    render3DController.onChange(render3D => {
        document.querySelector('#scatter-gl-container').style.display = render3D ? 'inline-block' : 'none';
    });
}
...
function showBackendFlagSettings(folderController, backendName) {
    ...
    for (let index = 0; index < tunableFlags.length; index++) {
        ...
        // When tunable (bool) and range (array) attributes of `flagRegistry` is implemented, we can apply them to here.
        ...
        if (typeof flagValueRange[0] == 'boolean') {
            ...
            flagController = flagController.add(params.STATE.flags, flag, flagValueRange);
 ...
 *
 *  - Else, clean up flag settings for the previous backend and show flag settings
...
async function showFlagSettings(folderController, backendName) {
    ...
    // The first controller under the `previousController` is the backend setting.
    const fixedSelectionCount = 1;
    while (folderController.__controllers.length > fixedSelectionCount) {
        folderController.remove(folderController.__controllers[folderController.__controllers.length - 1]);
    }
